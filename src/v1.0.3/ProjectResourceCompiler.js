...
const handleFile = (type, project, source) => {
    ...
    _targetFilename = source;

    if (type === "style" && project.processStyle) {
        let mainCSS = sass.compile(_targetFilename.source);

        try {
            fs.writeFileSync(outFile, mainCSS.css);

            let minCSS = sass.compileString(mainCSS.css, {
                style: "compressed"
            });
...
const handleBundle = (type, project) => {
    ...
    log("Bundle", `${type.toUpperCase()} => ${getFilename(bundlePath).replaceAll(" ", ".")}`);
